...
class AutoInterp:
    ...
    async def run_single_feature(
        ...
        (explanation_raw,), logs = await self.get_api_response(
            gen_prompts, self.cfg.max_tokens_in_explanation
        )
        ...
        if self.cfg.scoring:
            ...
            (predictions_raw,), logs = await self.get_api_response(
                scoring_prompts, self.cfg.max_tokens_in_prediction
            )
    ...
    async def get_api_response(
        ...
        """Generic API usage function for OpenAI with rate limiting"""
...
def run_eval(
    ...
    if peft_state_dict:
        ...
        state_dict = torch.load(peft_state_dict, map_location=device)
        base_model.load_state_dict(state_dict)
        print("PEFT weights loaded")
        ...
        print("Turned into HookedTransformer")
    else:
